...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        self.save_dir = opt.result_dir  # TODO: save_dir
        self.loss_names = []
        self.model_names = []
        self.optimizers = []
    ...
    def setup(self, opt):
        ...
        if self.isTrain:
            ...
            for name in self.model_names:
                ...
                # Set learning rate update network
                # if opt.model == 'meta_sgd':
                #     net_alpha = getattr(self, f"{name}_alpha")
                #     net_alpha = tools.init_net(net_alpha, opt.init_type, opt.init_gain, opt.gpu_ids)
                #     setattr(self, f"{name}_alpha", net)
        else:
            self.eval()
    ...
    def save_networks(self, index):
        if not os.path.exists(os.path.join(self.save_dir, str(index))):
            os.makedirs(os.path.join(self.save_dir, str(index)))
        for name in self.model_names:
            if isinstance(name, str):
                save_path = os.path.join(self.save_dir, str(index),'net_%s.pth' % (name))
                net = getattr(self, name)

                # save_alpha_path = os.path.join(self.save_dir, '%s_%s_net_alpha_%s.pth' % (task,type_ratio, name))
                # net_alpha = getattr(self, f"{name}_alpha")
                ...
                if len(self.gpu_ids) > 0 and torch.cuda.is_available():
                    ...
                    # torch.save(net_alpha.cpu().state_dict(), save_alpha_path)
                    # net_alpha.cuda(self.gpu_ids[0])
                else:
    ...
    def load_networks(self, filename):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                load_path = os.path.join(filename, load_filename)
                net = getattr(self, name)
